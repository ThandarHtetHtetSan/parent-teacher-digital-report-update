...
def admin_get_students():
    try:
        ...
        cursor = db.cursor(dictionary=True)
        cursor.execute("""
            SELECT s.id, s.name, s.student_code, s.class_id, s.user_id, s.academic_year
            FROM students s
            ORDER BY s.id DESC
        """)
        students = cursor.fetchall()
...
def admin_create_student():
    try:
        ...
        class_id = data.get('class_id')
        user_id = data.get('user_id')
        academic_year = data.get('academic_year')  # Add this line

        # Add academic_year to validation
        if not name or not student_code or not class_id or not user_id or not academic_year:
            return jsonify({"success": False, "message": "Missing required fields"}), 400
        ...
        cursor = db.cursor()
        cursor.execute("""
            INSERT INTO students (name, student_code, class_id, user_id, academic_year)
            VALUES (%s, %s, %s, %s, %s)
        """, (name, student_code, class_id, user_id, academic_year))  # Add academic_year
        db.commit()
        cursor.close()
        db.close()
        return jsonify({"success": True, "message": "Student created"}), 201
...
def admin_update_student(student_id):
    try:
        ...
        cursor = db.cursor()
        cursor.execute("""
            UPDATE students SET name=%s, student_code=%s, class_id=%s, user_id=%s, academic_year=%s WHERE id=%s
        """, (name, student_code, class_id, user_id, academic_year, student_id))  # Add academic_year
        db.commit()
        cursor.close()
        db.close()
        return jsonify({"success": True, "message": "Student updated"}), 200
