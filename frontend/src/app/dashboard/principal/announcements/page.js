...
export default function PrincipalAnnouncements() {
  ...
  const handleCreate = async (e) => {
    ...
    // Check if this is for parents
    const selectedRoleObj = roles.find(r => r.id === parseInt(form.target_role_id));
    
    if (selectedRoleObj && selectedRoleObj.name.toLowerCase() === 'parent') {
      // Send email using FormSubmit (no registration required)
      try {
        const formData = new FormData();
        formData.append('_replyto', 'noreply@school.com');
        formData.append('_subject', `New School Announcement: ${form.title}`);
        formData.append('message', `A new announcement has been made for parents:\n\n${form.body}\n\nThis is an automated message from the school.`);
        formData.append('_cc', 'khinwaihlaing734@gmail.com');
        
        // This will send the email
        await fetch('https://formsubmit.co/ajax/khinwaihlaing734@gmail.com', {
          method: 'POST',
          body: formData
        });
        
        alert('Email sent to parents!');
      } catch (emailError) {
        console.error('Email error:', emailError);
        alert('Announcement created but email failed to send.');
      }
    }
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center">
          <div className="bg-white rounded-xl p-6 w-full max-w-2xl">
            ...
            <form className="space-y-4" onSubmit={handleCreate}>
              ...
              {/* ADD THIS NOTIFICATION UI */}
              {roles.find(r => r.id === parseInt(form.target_role_id))?.name.toLowerCase() === 'parent' && (
                <div className="text-sm text-blue-600 bg-blue-50 p-3 rounded-lg">
                  <p>This announcement will be emailed to parents.</p>
                </div>
              )}
